...
func appendFront(x []int64, y int64) []int64 {
	x = append([]int64{y}, x...)
	return x
}
...
func extrapolate(hist []int64) int64 {
	...
	for i := len(diffs) - 1; i >= 0; i-- {
		if i == len(diffs)-1 {
			diffs[i] = appendFront(diffs[i], 0)
		} else {
			diffs[i] = appendFront(diffs[i], diffs[i][0]-diffs[i+1][0])
		}
	}
	fmt.Println(hist)
	...
	fmt.Printf("returning %d-%d=%d\n", hist[0], diffs[0][0], hist[0]-diffs[0][0])
	return hist[0] - diffs[0][0]
}
